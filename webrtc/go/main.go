...
// 	webrtc "github.com/John-LittleBearLabs/go-libp2p/p2p/transport/webrtc"
	...
	"github.com/testground/sdk-go/sync"
 	...
 	compat "github.com/John-LittleBearLabs/tg-libp2p/webrtc/go/compat"
)
...
// runWebrtc is the test case logic.
//
...
func runWebrtc(runenv *runtime.RunEnv, initCtx *run.InitContext) error {
	...
	// traffic shawebrtc rules are applied to the data network. Thanks to this
	...
	// This is a closure that webrtcs all peers in the test in parallel, and
	// records the latency value as a message and as a result datapoint.
	webrtcPeers := func(tag string) error {
		...
		for _, ai := range peers {
			...
			g.Go(func() error {
				...
				// record a message.
				runenv.RecordMessage("webrtc result (%s) from peer %s: %s", tag, id, res.RTT)

				// record a result point; these points will be batch-inserted
				// into InfluxDB when the test concludes.
				//
				// webrtc-result is the metric name, and round and peer are tags.
				point := fmt.Sprintf("webrtc-result,round=%s,peer=%s", tag, id)
	...
	// 📡  Let's webrtc all our peers without any traffic shawebrtc rules.
	if err := webrtcPeers("initial"); err != nil {
		return err
	}
	...
	for i := 1; i <= iterations; i++ {
		...
		if err := webrtcPeers(fmt.Sprintf("iteration-%d", i)); err != nil {
			return err
		}
